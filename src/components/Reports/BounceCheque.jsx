...
const BounceCheque = () => {
    ...
    const fetchBlocks = async () => {
        setLoading(true);
        try {
            const response = await axiosInstance.get('/realEstate/cheque-clearance/getAll-cheques-payment');
            if (response.data.status === 200 && Array.isArray(response.data.data)) {
                const emiPlanData = response.data.data.filter(item => item.chequeStatus === "clear" );
    ...
    useEffect(() => {
        ...
        if (filterDate) {
            filtered = filtered.filter(item =>
                item.updateDate?.includes(filterDate)
            );
        }

        if (filterBookingCode.trim()) {
            filtered = filtered.filter(item =>
                item.chequeDdNeftNo?.toLowerCase().includes(filterBookingCode.toLowerCase())
            );
        }
                ...
                <Table sx={{ minWidth: 650 }}>
                    <TableHead sx={{ backgroundColor: '#DAE1F3' }}>
                        <TableRow>
                            ...
                            <TableCell sx={{ fontWeight: 600 }}>Cheque No.</TableCell>
                        ...
                        {filteredBlocks.map((block) => (
                            <React.Fragment key={block.blockId}>
                                <TableRow>
                                    ...
                                    <TableCell sx={{ fontWeight: 500 }}>{block.blockNo}</TableCell>
                                    <TableCell sx={{ fontWeight: 500 }}>{block.plotNo}</TableCell>
                                    <TableCell sx={{ fontWeight: 500 }}>{block.chequeDdNeftNo}</TableCell>
                                    <TableCell sx={{ fontWeight: 500 }}>{block.amount}</TableCell>
                                    <TableCell sx={{ fontWeight: 500 }}>{block.chequeStatus}</TableCell>
                                    <TableCell sx={{ fontWeight: 500 }}>{block.updateDate}</TableCell>
